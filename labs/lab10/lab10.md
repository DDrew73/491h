...
Write your own h bridge driver library based on the datasheet
Drive a wheel forward and backward, with a function that maps speed to 0-100. 
What is the minimum duty cycle where the motor still turns? How does changing the PWM frequency change this?
Choose one of these two: 
-Grab your accelerometer and make a tilt controller for moving the motor forward/backward based on pitch of your breadboard. Set up a button that "emergency stops" the motor whenever it is pressed. 
-Set up your potentiometer so that you can control the speed from 0-100, forward and backward, with one full turn. Set up a button that "emergency stops" the motor whenever it is pressed. 
